...
class employee_voucher(models.Model):
    ...
    voucher_method2     = fields.Many2one('employee.voucher.line',string='Employee Payment Type2', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a2','=',True)]))
    ...
    voucher_method3     = fields.Many2one('employee.voucher.line',string='Employee Payment Type3', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a3','=',True)]))
    ...
    voucher_method5     = fields.Many2one('employee.voucher.line',string='Employee Payment Type5', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a5','=',True)]))
    ...
    voucher_method9     = fields.Many2one('employee.voucher.line',string='Employee Payment Type9', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a9','=',True)]))
    ...
    context_2     = fields.Many2one('employee.voucher.line',string='Context', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a2','=',True)]))
    context_3     = fields.Many2one('employee.voucher.line',string='Context', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a3','=',True)]))
    context_5     = fields.Many2one('employee.voucher.line',string='.', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a5','=',True)]))
    context_9     = fields.Many2one('employee.voucher.line',string='.', readonly=True, default=lambda self: self.env['employee.voucher.line'].search([('a9','=',True)]))        
...
################## Voucher Cancel Functions ############################################################
    ...
    def cancel_voucher(self):    
        ...
        for payment_slip in self:
            ...
            if payment_slip.ticket_leave == True:
                inv_remain_ticket = self.get_inv_remaining_tickets()
                if payment_slip.voucher_method2.account_analytic_true == True:
                    ...
                    if debit_account_id2:
                        debit_line2 = (0, 0, {
                        'name': payment_slip.voucher_method2.name,
                        'date': timenow,
             ...
             #   print "VOVOVOVOVOVOVVVOVOVOOVVO",emp_voucher_ids
                ...
                if payment_slip.voucher_method3.account_analytic_true == True:
                    ...
                    if debit_account_id3:
                        debit_line3 = (0, 0, {
                        'name': payment_slip.voucher_method3.name,
                        'date': timenow,
                        'partner_id': payment_slip.employee_id.address_home_id.id,
            ...
            if payment_slip.deduct1 == True:
                ...
                if payment_slip.voucher_method5.account_analytic_true == True:
                    ...
                    if debit_account_id5:
                        debit_line5 = (0, 0, {
                        'name': payment_slip.voucher_method5.name,
                        'date': timenow,
                        'partner_id': payment_slip.employee_id.address_home_id.id,
            ...
            if payment_slip.other == True:
                ...
                if payment_slip.voucher_method9.account_analytic_true == True:
                    ...
                    if debit_account_id9:
                        debit_line9 = (0, 0, {
                        'name': payment_slip.voucher_method9.name,
                        'date': timenow,
                        'partner_id': payment_slip.employee_id.address_home_id.id,
    ...
    def approve2_payment(self):    
        ...
        for payment_slip in self:
            ...
            if payment_slip.ticket_leave == True:
                ticket_1 = payment_slip.issue_amount_tickets
                grand_tot.append(ticket_1)
                if payment_slip.issue_tickets > payment_slip.allocated_tickets:
                    raise UserError(_('No Sufficent Ticket Balance for this Employee'))    
                remain_ticket = self.get_remaining_tickets()
                if payment_slip.voucher_method2.account_analytic_true == True:
                    ticket_analytic = payment_slip.employee_id.analytic_account_id and payment_slip.employee_id.analytic_account_id.id or False
                    if debit_account_id2:
                        debit_line2 = (0, 0, {
    ...
    def onchange_employee_id(self):
      ...
      ########### Calculation of Employee TERMINATION #############      
                ...
                if eos_term_years < 2 :
                    ...
                    term_second = 5*emp_net/2
                    term_third = term_rem_val * emp_net
      ...
      ########### Calculation of Employee QUIT #############      
                ...
                if eos_quit_years < 2 :
                    ...
                    quit_rem_val = eos_quit_years - 5
                    #print "R3R3R3R3R3R3R3R3R3R3R3R3R3",quit_rem_val
                    quit_second = (5*emp_net/2)*2/3
                    quit_third = (quit_rem_val*emp_net)*2/3
                    #print "Q3Q3Q3Q3Q3Q3Q3Q3Q3Q3Q3Q3",quit_third
                    
                elif eos_quit_years >= 10:
                    quit_rem_value = eos_quit_years - 5
                    #print "R4R4R4R4R4R4R4RR44R4R4R4",quit_rem_value
                    quit_second = 5*emp_net/2
                    quit_fourth = quit_rem_value*emp_net
